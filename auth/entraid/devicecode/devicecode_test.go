...
var cnfgPath = "./config/private.spo-entraid-devicecode.json"
...
func TestParseConfig(t *testing.T) {
	auth := &AuthCnfg{}
	err := auth.ParseConfig([]byte(`{"siteUrl":"https://contoso.sharepoint.com", "tenantId":"contoso", "clientId":"mocked-client"}`))
...
func TestIntegration_FullAuthFlow(t *testing.T) {
	...
	if deviceCodeFile := os.Getenv("DEVICE_CODE_FILE"); deviceCodeFile != "" {
		auth.UserPrompt = func(_ context.Context, msg azidentity.DeviceCodeMessage) error {
			return os.WriteFile(deviceCodeFile, []byte(msg.Message+"\n"), 0o644)
		}
	}
